# Memcached Cluster Setup Documentation

## Overview
This document outlines the setup of a Memcached cluster using Docker, Docker Compose, and mcrouter. The cluster consists of 9 Memcached instances and 3 mcrouter instances, designed to provide efficient caching, load balancing, and fault tolerance for web applications.
...
### Docker Compose File
The configuration is defined in a `docker-compose-cluster-3-9.yml` file, which is used to create and manage the cluster's services.
...
```bash
docker-compose -f docker-compose-cluster-3-9.yml up -d
```
...
```bash
docker-compose -f docker-compose-cluster-3-9.yml down
```
